...
1. Check `.opencode/bee-state.local.md` for an in-progress discovery session.
...
- Any triage assessment and context summary (when invoked from `/bee-sdd`)
- Mode hint: "standalone" (when invoked directly) or "from-bee" (when invoked via `/bee-sdd` orchestrator)
...
3. Update `.opencode/bee-state.local.md` with discovery doc path and phase: "discovery complete".
