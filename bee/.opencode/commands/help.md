...
"Bee also ships with **skills** — shared reference knowledge that agents import automatically. You can also load them yourself by name to read up on a topic:
...
- `spec-writing` — Acceptance criteria, vertical slicing
- `design-fundamentals` — Accessibility, typography, spacing, responsive breakpoints
- `debugging` — Systematic diagnosis: read before you change, assume nothing
- `ai-workflow` — Why spec-first TDD produces better AI code
- `collaboration-loop` — Inline review with `@bee` annotations
- `code-review` — Review methodology, hotspot analysis
- `ai-ergonomics` — Making code LLM-friendly
- `browser-testing` — Chrome MCP tool reference, screenshot conventions

On opencode, type `/<skill-name>` to load any of them (e.g., `/clean-code`). On Claude Code, ask me to load the skill by name. These are read-only references — no code changes."
