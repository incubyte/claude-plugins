...
2. Ask the developer for consent using question:
   "Discovery produced module boundaries for this project. Want me to save these so Claude respects them in all code it writes? I'll create `.opencode/BOUNDARIES.md`."
   Options: "Yes, save boundaries (Recommended)" / "No, skip for now"

3. If the developer consents, generate `.opencode/BOUNDARIES.md` in the **target project** (not the bee plugin directory) with this format:
...
4. If `.opencode/BOUNDARIES.md` already exists (from a previous discovery run), ask:
...
5. If `AGENTS.md` exists in the target project and doesn't already reference `BOUNDARIES.md`, ask:
   "Want me to add a reference to BOUNDARIES.md in your AGENTS.md so Claude always has boundary context?"
   Options: "Yes, add reference (Recommended)" / "No, skip"
   If yes, append a line to AGENTS.md: `Read .opencode/BOUNDARIES.md for module boundary rules.`

6. Report to the developer which modules and boundaries were saved.
   **→ Update state:** `BEE_DIR=.opencode "$HOME/.config/opencode/bee/scripts/update-bee-state.sh" set --boundaries ".opencode/BOUNDARIES.md"`
